...
LOG_FILE="/var/log/gpg_encrypt_tar.log"
# ===========================
...
echo "=== Starting TAR.GZ encryption at $(date) ===" | tee -a "$LOG_FILE"
...
echo "Pruning old TAR.GZ encrypted files, keeping only $MAX_KEEP..." | tee -a "$LOG_FILE"
mapfile -t ENCRYPTED_FILES < <(find "$TARGET_DIR" -type f -name "*.tar.gz.gpg" -printf "%T@ %p\n" | sort -nr | awk '{print $2}')
...
echo "=== TAR.GZ encryption completed at $(date) ===" | tee -a "$LOG_FILE"
